...
const releaseTracker = read('docs/RELEASE_V1_GO_LIVE_TRACKER.md');
...
assert(goLiveCertificate.includes('#26 — runtime image security scan/remediation complete'), 'go-live certificate requires runtime-image security gate #26');
assert(goLiveCertificate.includes('## Runtime security exception requirement'), 'go-live certificate defines scoped runtime-security exception requirements');
assert(goLiveCertificate.includes('exploitability/exposure') && goLiveCertificate.includes('target remediation date') && goLiveCertificate.includes('explicit Go/No-Go approval reference'), 'go-live certificate requires accountable runtime-security exception evidence');
assert(goLiveCertificate.includes('#26 runtime security evidence / exception approval | PENDING'), 'go-live certificate keeps #26 evidence pending until explicitly resolved');
assert(releaseTracker.includes('#26 — P0 scan/remediate release runtime images'), 'release tracker includes runtime security gate #26');
assert(releaseTracker.includes('No open P0 at launch, including #26.'), 'release tracker forbids launch while #26 is open');
